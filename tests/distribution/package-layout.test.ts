...
describe("npm distribution layout", () => {
  ...
  test("platform packages constrain installation to their target", async () => {
    ...
    for (const target of targets) {
      ...
      // npm provenance (generated by Trusted Publishing) rejects packages whose
      // repository.url does not match the publishing GitHub repository.
      expect(packageJson.repository).toEqual({
        type: "git",
        url: "git+https://github.com/Toubat/agentic-debug-mode.git",
      });
    }
  });

  test("launcher declares the provenance-matching repository", async () => {
    const launcher = await manifest(join(root, "packages", "npm-launcher", "package.json"));
    expect(launcher.repository).toEqual({
      type: "git",
      url: "git+https://github.com/Toubat/agentic-debug-mode.git",
    });
  });
