...
import {
  type EnsureDaemonFunction,
  ensureDaemon,
  withTransientDaemonRetry,
} from "../cli/daemon-manager";
...
export async function createCommand(
  ...
  try {
    const created = await withTransientDaemonRetry(async () => {
      const daemon = await ensure({
        homeDirectory: process.env.AGENT_DEBUG_MODE_HOME_OVERRIDE,
      });
      return requestDaemonControl<SessionIngest>(daemon, "/v1/control/sessions", {
        method: "POST",
      });
    });
    return {
