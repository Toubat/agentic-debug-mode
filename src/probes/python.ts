...
export function renderPythonTemplate(): ProbeTemplates {
  const helperTemplate = [
    ...
    '        r"(^|_)(password|passwd|pwd|secret|token|api_key|authorization|authorization_header|cookie|set_cookie|private_key|client_secret|access_token|refresh_token|id_token|auth_token|bearer_token|credential|credentials)$",',
    "        normalized,",
    "    ) is not None",
    "",
    "def __agent_debug_redact(value):",
    "    if isinstance(value, list):",
    "        return [__agent_debug_redact(item) for item in value]",
    "    if isinstance(value, dict):",
    "        return {",
    '            key: "[REDACTED]" if __agent_debug_is_secret_key(key) else __agent_debug_redact(item)',
